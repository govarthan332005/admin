...
window.switchPage = function(page, link) {
  ...
  var titles = {
    ...
    "daily-returns": "Auto Daily Returns"
  };
...
async function loadDepositSettings() {
  ...
  // Load deposit config (QR + Bank + toggles)
  try {
    ...
    if (snap.exists()) {
      ...
      if (data.bankName) document.getElementById('cfgBankName').value = data.bankName;
      if (data.bankAccountName) document.getElementById('cfgBankAccountName').value = data.bankAccountName;
      if (data.bankAccountNumber) document.getElementById('cfgBankAccountNumber').value = data.bankAccountNumber;
      if (data.bankIfsc) document.getElementById('cfgBankIfsc').value = data.bankIfsc;
    }
  } catch(e) { console.warn('Deposit config load:', e); }
}
...
window.saveBankDepositConfig = async function() {
  try {
    ...
    await backend.setDoc(backend.doc(backend.db, 'config', 'deposit'), Object.assign({}, prev, {
      ...
      bankAccountName: document.getElementById('cfgBankAccountName').value.trim(),
      bankAccountNumber: document.getElementById('cfgBankAccountNumber').value.trim(),
      bankIfsc: document.getElementById('cfgBankIfsc').value.trim(),
      updatedAt: backend.serverTimestamp()
    }));
    showToast('Bank deposit details saved! ✅', 'success');
  } catch(err) { showToast('Failed: ' + err.message, 'error'); }
};
...
async function loadLinksSettings() {
  try {
    ...
    if (snap.exists()) {
      var data = snap.data();
      if (data.telegramLink) document.getElementById('cfgTelegramLink').value = data.telegramLink;
      if (data.customerCareLink) document.getElementById('cfgCustomerCareLink').value = data.customerCareLink;
    }
...
window.processAllUsersDailyReturns = async function() {
  ...
  try {
    ...
    for (var u = 0; u < usersSnap.docs.length; u++) {
      ...
      if (investSnap.empty) continue;

      var userDailyCredit = 0;
      var userPlansCompleted = 0;
      ...
      for (var i = 0; i < investSnap.docs.length; i++) {
        ...
        // Skip plans created after today's cutoff
        var createdAt = inv.createdAt || inv.startDate;
        if (createdAt) {
          var createdDate = createdAt.toDate ? createdAt.toDate() : new Date(createdAt);
        ...
        var dailyReturn = inv.dailyReturnFixed || 0;
        userDailyCredit += dailyReturn;

        var newDays = daysCompleted + 1;
        var updateData = { daysCompleted: newDays, lastDisbursedDate: today };
        if (newDays >= duration) {
          updateData.status = 'completed';
          userPlansCompleted++;
        }
        await backend.updateDoc(backend.doc(backend.db, 'investments', invDoc.id), updateData);
      }

      if (userDailyCredit > 0) {
        ...
        if (freshSnap.exists()) {
          ...
          await backend.updateDoc(backend.doc(backend.db, 'users', userId), {
            balance: (fd.balance || 0) + userDailyCredit,
            withdrawableBalance: (fd.withdrawableBalance || 0) + userDailyCredit,
            totalReturns: (fd.totalReturns || 0) + userDailyCredit,
            activePlans: Math.max(0, (fd.activePlans || 0) - userPlansCompleted)
          });

          await backend.addDoc(backend.collection(backend.db, 'transactions'), {
            userId: userId, type: 'daily_return', amount: userDailyCredit,
          ...
          await backend.addDoc(backend.collection(backend.db, 'notifications'), {
            userId: userId,
            message: '💰 Daily returns of ₹' + userDailyCredit + ' auto-credited to your wallet!',
            type: 'success', read: false, createdAt: backend.serverTimestamp()
          });

          totalCredited += userDailyCredit;
          usersProcessed++;
          addLog('✅ ' + (userData.name || userId) + ': ₹' + userDailyCredit + ' credited');
        }
      } else if (userPlansCompleted > 0) {
        var freshSnap2 = await backend.getDoc(backend.doc(backend.db, 'users', userId));
        if (freshSnap2.exists()) {
          var fd2 = freshSnap2.data();
          await backend.updateDoc(backend.doc(backend.db, 'users', userId), {
            activePlans: Math.max(0, (fd2.activePlans || 0) - userPlansCompleted)
          });
        }
      }

      plansCompleted += userPlansCompleted;
    }
...
// ═══ AUTO-PROCESS daily returns when admin opens dashboard ═══
(function autoProcessDailyReturns() {
  // Wait for dashboard to be ready, then auto-process
  var checkInterval = setInterval(function() {
    if (adminUser && adminData) {
      clearInterval(checkInterval);
      // Small delay to let dashboard load first
      setTimeout(function() {
        console.log('[Admin] Auto-processing daily returns...');
        processAllUsersDailyReturns();
      }, 3000);
    }
  }, 1000);
