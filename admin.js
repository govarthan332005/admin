...
async function loadWithdrawSettings() {
  try {
    ...
    updateWithdrawUI(enabled);
  } catch(e) { console.warn('Withdraw settings load:', e); }
}

function updateWithdrawUI(enabled) {
...
window.toggleApiWithdraw = async function() {
  ...
  try {
    await backend.setDoc(backend.doc(backend.db, 'config', 'withdraw'), {
...
console.log("[Admin] SliceInvest Admin Panel v1.1 (UPDATED) script loaded");
