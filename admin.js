...
async function loadAllTransactions() {
  ...
  try {
    ...
    tbody.innerHTML = docs.map(function(d) {
      var data = d.data();
      // v17 FIX: distinct colours and labels for daily_reward / signup_bonus / admin_credit
      var color = '#6C5CE7';
      var label = data.type;
      if (data.type === 'deposit')         { color = '#00B894'; label = 'Deposit'; }
      else if (data.type === 'withdraw')   { color = '#E74C3C'; label = 'Withdraw'; }
      else if (data.type === 'daily_return') { color = '#10B981'; label = 'Daily Return'; }
      else if (data.type === 'daily_reward') { color = '#F59E0B'; label = 'Daily Reward'; }
      else if (data.type === 'referral_bonus') { color = '#FD79A8'; label = 'Referral'; }
      else if (data.type === 'signup_bonus')  { color = '#EC4899'; label = 'Signup Bonus'; }
      else if (data.type === 'admin_credit')  { color = '#06B6D4'; label = 'Admin Credit'; }
      else if (data.type === 'invest')        { color = '#6366F1'; label = 'Invest'; }
      var badgeClass = (data.type === 'deposit' || data.type === 'daily_return' || data.type === 'daily_reward' || data.type === 'signup_bonus' || data.type === 'admin_credit') ? 'approved' : data.type === 'withdraw' ? 'rejected' : 'active';
      return '<tr><td data-label="User ID" style="font-size:11px;max-width:120px;overflow:hidden;text-overflow:ellipsis">' + (data.userId || "\u2014") + '</td><td data-label="Type"><span class="status-badge ' + badgeClass + '">' + label + '</span></td><td data-label="Amount"><strong style="color:' + color + '">' + fmt(data.amount) + '</strong></td><td data-label="Status"><span class="status-badge ' + data.status + '">' + data.status + '</span></td><td data-label="Date">' + fmtDateTime(data.createdAt) + '</td><td data-label="Details" style="font-size:11px;color:#94A3B8">' + (data.plan || data.description || data.reference || data.account || data.method || data.note || "\u2014") + '</td></tr>';
    }).join("");
  } catch(err) {
    console.error(err);
    tbody.innerHTML = '<tr><td colspan="6" class="empty-cell">Error: ' + err.message + '</td></tr>';
  ...
  container.innerHTML = adminPlansCache.map(function(p, idx) {
    ...
    return '<div class="plan-mgmt-card">' +
      ...
      '<div class="plan-mgmt-details">' +
        ...
        '<div class="plan-mgmt-row"><span>Investors Shown</span><strong>' + (p.investorCount !== undefined && p.investorCount !== null && p.investorCount !== '' ? p.investorCount : '<em style="color:var(--text-3);font-style:normal;font-weight:600;">Auto</em>') + '</strong></div>' +
...
window.openAddPlanModal = function() {
  ...
  // v12 admin: investor count (empty = auto)
  var icEl = document.getElementById('planInvestorCount');
  if (icEl) icEl.value = '';
...
window.editPlan = function(index) {
  ...
  // v12 admin: investor count
  var icEl = document.getElementById('planInvestorCount');
  if (icEl) icEl.value = (p.investorCount !== undefined && p.investorCount !== null) ? String(p.investorCount) : '';
...
window.savePlan = async function() {
  ...
  // v12 admin: capture investor count (empty string → do not set, fallback to auto in user app)
  var icRaw = (document.getElementById('planInvestorCount') && document.getElementById('planInvestorCount').value || '').trim();
  var planData = {
  ...
  if (icRaw !== '' && !isNaN(Number(icRaw))) {
    planData.investorCount = Math.max(0, Math.floor(Number(icRaw)));
  } else {
    planData.investorCount = null; // explicitly null → auto in user app
  }
