...
async function checkAndPayReferralBonus(userId, depositAmount) {
  if (depositAmount < 500) return; // Only for deposits >= ₹500
  try {
    var userRef = backend.doc(backend.db, 'users', userId);
    var userSnap = await backend.getDoc(userRef);
    ...
    // Load platform config for referral bonus amount
    var configSnap = await backend.getDoc(backend.doc(backend.db, 'config', 'platform'));
    var referralBonus = 50;
    if (configSnap.exists() && configSnap.data().referralBonus) {
      referralBonus = Number(configSnap.data().referralBonus);
    }
    ...
    await backend.addDoc(backend.collection(backend.db, 'notifications'), {
      ...
      message: '🎉 ' + (userData.name || 'Someone') + ' deposited using your referral! ' + fmt(referralBonus) + ' credited to your wallet.',
      type: 'success', read: false, createdAt: backend.serverTimestamp()
    });

    // Pay referred user too — add to both balance AND withdrawableBalance so it can be withdrawn
    await backend.updateDoc(userRef, {
      balance: (userData.balance || 0) + referralBonus,
      withdrawableBalance: (userData.withdrawableBalance || 0) + referralBonus,
      referralBonusPaid: true
    });
    await backend.addDoc(backend.collection(backend.db, 'notifications'), {
      userId: userId,
      message: '🎉 Referral bonus of ' + fmt(referralBonus) + ' credited! Thanks for your first deposit.',
      type: 'success', read: false, createdAt: backend.serverTimestamp()
    });

    console.log('[Admin] Referral bonus paid for user ' + userId);
  } catch(e) {
...
console.log("[Admin] SliceInvest Admin Panel v3.0 (UPDATED) script loaded");
